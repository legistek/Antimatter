...
namespace AntimatterJS.Sample.AppModel
{
    ...
    {
        ICommand[] _FileCommands;
        public ICommand[] FileCommands =>
            _FileCommands ?? (_FileCommands = new ICommand[]
            {
                SelectFileCommand
            });
        ...
        public ICommand SelectFileCommand
        {
            get
            {
                return _SelectFileCommand ?? (_SelectFileCommand = new Command(
                    async (arg) =>
                    {
                        var reactor = Reactor.GetFor(this, out _);
                        if (reactor == null)
                            return;
                ...
                return _LoadFileCommand ?? (_LoadFileCommand = new Command(
                    async (arg) =>
                    {
                        ...
                        try
                        {
                            ...
                            using (var str = this.SelectedFile.OpenRead())
                            {
